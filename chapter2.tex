\section{Topology, manifolds, links and blackboard framed links}
\label{chap:topologyEtc}

\subsection{Topology, manifolds and what we call here ``spaces''}
\label{sec:topology}
...
based on the clean and direct approach of \cite{viro2008elementary},
...
are {\em manifolds} and the specific class of closed, connected,
...
\begin{figure}[htp]
   \begin{center}
      \leavevmode
      \includegraphics[width=5cm]{E.figsbw2/sevenbridges.pdf}
      \hspace{1cm}
      \includegraphics[width=5cm]{A.figs/sevenbridgeswrongwalk.eps} \\
   (A) \hspace{5.5cm} (B)\\
   \end{center}
   \vspace{-0.7cm}
...
Topology, in its present form, long after Euler, uses the term {\em
topological space} for what we called a ``shape'' on the beginning
of this section. Before defining what are {\em topological spaces}
we define {\em metric spaces}, as they are the source for the
concrete ``shapes'' or {\em topological spaces} that we are
interested.

\bigskip \centerline{\bf \textsc{Metric Spaces}} \bigskip

A {\em metric} or a {\em distance} in a set $X$ is a function $\rho:
X \times X \rightarrow \mathbb{R}_+ = \{x \in \mathbb{R} \, | \, x \geq 0 \}$ that
satisfies $$\begin{array}{l}
  \hbox{(1) \,} \rho(x,y) = 0, \hbox{ iff }x = y,\\[0.2cm]
  \hbox{(2) \,}\rho(x,y) = \rho(y,x), \hbox{ for every }x,y \in X,\\[0.2cm]
  \hbox{(3) \,}\rho(x,y) \leq \rho(x,z) + \rho(z,y), \hbox{ for every }x,y,z \in X. 
  \hbox{ ({\em triangle inequality})}\\
\end{array}$$
The pair $(X,\rho)$, where $\rho$ is a metric in $X$, is called a
{\em metric space}. The function
$$\mathbb{R}^n \times \mathbb{R}^n \rightarrow \mathbb{R}_{+} : (x,y) \mapsto
\sqrt{\sum_{i=1}^{n}{(x_i-y_i)^2}}$$ is a metric in $\mathbb{R}^n$ and is
called {\em euclidean} metric.
...
\end{array}$$
are called, respectively, {\em open ball}, {\em closed ball}, and
{\em sphere} of the space $(X,\rho)$ with center $a$ and radius $r$.
If $(X,\rho)$ is a metric space and $A \subset X$, then the
restriction of metric $\rho$ to $A \times A$ is a metric in $A$, and
$(A,\rho|_{A \times A})$ is a metric space. It is called a {\em
subspace} of $(X,\rho)$. The ball $D_1(0)$ and the sphere $S_1(0)$
in $\mathbb{R}^n$ with the euclidean metric are denoted by symbols $D^n$
and $S^{n-1}$ and called {\em $n$-dimensional ball} and {\em
$(n-1)$-dimensional sphere}. They are considered as metric spaces
with the metric restricted to $\mathbb{R}^n$. Note that: $D^1$ is the
segment $[-1,1]$; $D^2$ is a disk; $S^0$ is the pair of points
\{-1,1\}; $\mathbb{S}^1$ is a circle; $\\mathbb{S}^2$ is a sphere; $D^3$ is a ball. The
words disk, circle, sphere and ball were used, in last sentence,
appealing to their common sense meaning. Now, for this work,
they have a formal meaning: a {\em disk} is $D^2$, a {\em circle} is
$\mathbb{S}^1$, a {\em sphere} is $\mathbb{S}^2$ and a {\em ball} is $B^3$.

\bigskip \centerline{\bf \textsc{Topological Spaces}} \bigskip

A {\em topological space} is a set $X$ with a collection $\Omega$ of
...
The collection $\Omega$ is called a {\em topological structure} or a
{\em topology} in $X$. The sets in $\Omega$ are called {\em open}.
The elements of $X$ are called {\em points}. A set $F \in X$ is said
{\em closed} in the space $(X,\Omega)$ if its complement $X
\backslash F$ is open ({\em i.e. $X \backslash F \in \Omega$}). Note
that $\varnothing$ and $X$ are both open and closed. A {\em
neighborhood} of a point is any open set containing that point. A
collection $\Sigma$ of open sets is called a {\em base} for a
topology ({\em i.e.} topological structure), if each nonempty open
set is a union of sets belonging to $\Sigma$.

The following result connects metric spaces and topological spaces:
\begin{center}
\em the collection of all open balls in a metric space $(X,\rho)$
\linebreak is a base for some topology in $X$.
\end{center}
For example, consider $\mathbb{R}^2$ with the euclidean metric. Then, a
topology for $\mathbb{R}^2$ is the set of all unions of open balls (open
...
\end{center}
The pair $(A,\Omega_A)$ is called a {\em subspace} of the space
$(X,\Omega)$. The collection $\Omega_A$ is called the {\em subspace
topology} or the topology {\em induced} on $A$ by $\Omega$, and its
elements are the open sets in $A$.

At this point, we can think, for instance, of $\mathbb{S}^2$ as a topological
space. We know that the collection of open balls of $\mathbb{R}^3$ (as a
metric space with the euclidean metric) is a base for a topology in
$\mathbb{R}^3$. Consider this topology to view $\mathbb{R}^3$ as a topological
space. Restrict this topology of $\mathbb{R}^3$ to $\mathbb{S}^2$ to obtain a
topology for $\mathbb{S}^2$: $\mathbb{S}^2$ is now a topological space. In this work
this logical sequence to obtain a topology for a subset of $\mathbb{R}^n$
is always the one considered. So, from now on, every subset of
$\mathbb{R}^n$ may also be viewed as a topological space. For example the
surface of doughnut and of the coffee cup considered in the
beginning of this section may now be viewed as subsets of $\mathbb{R}^3$
and, consequently, as topological spaces.

\bigskip \centerline{\bf \textsc{Maps}} \bigskip

In the context of topology, the terms {\em map} and {\em mapping}
are synonyms of function. A mapping $f: X \rightarrow Y$ is called a
{\em surjective map}, or just a {\em surjection} if every element of
$Y$ is an image of at least one element of $X$. It is called an {\em
injective map}, {\em injection} or {\em one-to-one map} if every
element of $Y$ is an image of, at most, one element of $X$. A
mapping is called a {\em bijective map}, {\em bijection}, or {\em
invertible} if it is surjective and injective.

The {\em image} of a set $A \subset X$ under a map $f: X \rightarrow
Y$ is the set of images of all points of $A$. It is denoted by
$f(A)$. Thus,
$$f(A) = \{ f(x) : x \in A \}.$$
The image of the entire set $X$ ({\em i.e.} $f(X)$) is called the
{\em image} of $f$. The {\em preimage} of a subset of $B \subset Y$
...
said to be {\em continuous} if the preimage of any open subset of
$Y$ is an open subset of $X$. A map $f:X \rightarrow Y$ is said to
be {\em continuous at point }$a \in X$ if for every neighborhood $U$
...
concept. An invertible mapping is called a {\em homeomorphism} if it
is continuous and its inverse is also continuous. A topological
space $X$ is said to be {\em homeomorphic} to space $Y$ if there is
a homeomorphism $X \rightarrow Y$. Being homeomorphic is {\em an
...
$[a,b]$ for any $a < b$; $(-1,1)$ and $\mathbb{R}$; an open disk and the
plane $\mathbb{R}^2$; $\mathbb{S}^n \backslash \{\hbox{point in }\mathbb{S}^n\}$ and $\mathbb{R}^n$.
Some examples of non-homeomorphic topological spaces: balls $D^p,
D^q$ with $p \neq q$; spheres $S^p, S^q$ with $p \neq q$; punctured
plane $\mathbb{R}^2 \backslash \{\hbox{point}\}$ and a plane with a hole
$\mathbb{R}^2 \backslash \{(x,y):x^2+y^2<1\}$.
...
spaces are {\em topologically equivalent} or {\em the same for the
...
the correct language, the topological equivalence problem or {\em
homeomorphism problem} is one of the classic and important problems
of topology:
\begin{center}
\em Given two topological spaces, are they homeomorphic?
\end{center}
...
A collection $\Gamma$ of subsets of a set $X$ is called a {\em
cover} or a {\em covering} if the union of the elements of
$\Gamma$ contains $X$, {\em i.e.}, $X \subset \cup_{A \in \Gamma}A$.
A cover $\Gamma$ of a topological space $X$ is said to be an {\em
open cover} if every element of $\Gamma$ is an open set. A cover
$\Gamma$ of a topological space $X$ is said to be a {\em closed
cover} if every element of $\Gamma$ is a closed set. If $\Sigma$
covers $X$ and $\Gamma$ covers $X$ and $\Sigma \subset \Gamma$, then
$\Sigma$ is a {\em subcover} or {\em subcovering} of $\Gamma$.

\bigskip \centerline{\bf \textsc{Connectedness}} \bigskip

A topological space $X$ is said to be {\em connected} if it has only
...
intuitive. Let's get a more intuitive definition. A {\em partition}
of a set is a cover of this set with pairwise disjoint sets. To {\em
partition} a set means to construct such a cover. Now the equivalent
notion of connectedness of a topological space:
\begin{center}
\em A topological space is connected iff it cannot be \linebreak
...
({\em e.g}: one open ball $B_1(-1,-1)$ and $B_1(1,1)$). This
topological space is not connected, because the two open disks, that
are open sets, form a partition of the entire space.

A {\em connected component} of a space $X$ is a maximal connected
subset of $X$ ({\em i.e.} a connected subset, that is not contained
...
A topological space $X$ is said to be {\em compact} if any of its
open covers has a finite subcover, {\em i.e.} if $\Gamma$ is a cover
...
space: $\mathbb{R}^n$. Example of a compact space: $\mathbb{S}^n$. Indeed a 
subset of $\mathbb{R}^n$ is compact
if and only if it is closed and bounded (i.e. contained in an open ball).
...
$x \in X$. Then $f$ and $g$ are said to be {\em homotopic} and $H$
is called a {\em homotopy} between $f$ and $g$. Homotopy of maps is
...
connecting $h$ and $h'$ ({\em i.e.}, with $h_0 = h$, $h_1 = h'$) is
called a {\em isotopy} between $h$ and $h'$ if $h_t$ is a
homeomorphism for each $t \in [0,1]$. Homeomorphisms $h, h'$ are
said to be {\em isotopic} if there exists an isotopy between $h$ and
...
connecting $h$ and $h'$ ({\em i.e.}, with $h_0 = h$, $h_1 = h'$) is
called an {\em (embedding) isotopy} between $h$ and $h'$ if $h_t$ is
an embedding for each $t \in [0,1]$. Embeddings $h, h'$ are said to
be {\em isotopic} if there exists an isotopy between $h$ and $h'$.
...
is called an {\em isotopy of the set $A = A_0$} if the graph $\Gamma
= \{(x,t) \in X \times I \, | \, x \in A_t\}$ of the family is {\em
fibrewise homeomorphic} to the cylinder $A \times I$, {\em i.e.}
...
said to be {\em ambient} for $\Phi_t$. Two isotopic subsets of a
topological space may not be ambient isotopic. Any pair of circles
$\mathbb{S}^1$ embedded in $\mathbb{S}^3$ is isotopic, but a circle
...
{\em locally euclidean space of dimension $n$} if each point of $X$
has a neighborhood homeomorphic either to $\mathbb{R}^n$ or $\mathbb{R}^n_{+}$
({\em i.e.} $\mathbb{R}^n_+ = \{ x \in \mathbb{R}^n : x_1 \geq 0 \}$, defined for
$n \geq 1$). Examples of locally euclidean spaces: $\mathbb{R}^n$; $\mathbb{S}^n$,
$D^n$.

A topological space is called {\em Hausdorff space} or just {\em
Hausdorff} if any two distinct points possess disjoint
neighborhoods. Result: any metric space is Hausdorff ({\em i.e.} the
topological space with topology induced from the metric space is
Hausdorff).

A space is said to satisfy the {\em second axiom of
countability} or to be {\em second countable} if it has a countable
base. Result: any metric space is Hausdorff ({\em i.e.} the
topological space with topology induced from the metric space has
a countable base).

A {\em manifold of dimension $n$} or $n$-manifold is a topological
...
Examples of {i.e. $n$-manifolds}: $\mathbb{R}^n$; $\mathbb{S}^n$, $D^n$.

The definitions until now were very formal, but this one will not be
formal. A manifold of dimension $n$ is called {\em non-orientable}
...
is not non-orientable is {\em orientable}. An orientable space has
...
\begin{center}
\em \large connected, closed, oriented 3-manifolds.
\end{center}
The adjective {\em closed} applied to a 3-manifold means that it is boundary free and compact.
We use, from now on, the word {\em space} to denote these
...
\begin{center}
\em \large A {\huge space} in this work is the same as \linebreak a
connected, closed, oriented 3-manifold.
\end{center}

\subsection{Knots, links and diagrams}
\label{sec:knotsAndLinks}

In general terms, {\em knot theory} studies the {\em placement
problem}. As stated in \cite{Kauffman1987}, this problem is
\begin{center}
\em Given topological spaces $X$ and $Y$, classify how $X$ may be
...
\end{center} When $X$ is the circle $\mathbb{S}^1$ and $Y$ is
the 3-dimensional space $\mathbb{R}^3$ or $\mathbb{S}^3$, we have {\em classical
...
An embedding of a circle $\mathbb{S}^1$ in the 3-dimensional space $\mathbb{R}^3$ or
in the 3-sphere $\mathbb{S}^3$ is called a {\em knot}. An embedding of a
collection of circles in the 3-dimensional space $\mathbb{R}^3$ or in the
3-sphere $\mathbb{S}^3$ is called a {\em link}. Each circle (or the image of
one) in a link is called a {\em component} of the link. So, a knot
...
suggestively called {\em unknot}. Figure \ref{fig:links3d}B is the
knot called {\em trefoil}. Figure \ref{fig:links3d}C is the knot
called {\em figure eight knot}. Figure \ref{fig:links3d}D is a
link with two components. Figure \ref{fig:links3d}E is a link with
three components and it is called the {\em borromean rings}. This link has
an interesting property: we cannot separate the three rings without
breaking one of them, {\em i.e.} the three rings are {\em linked},
even though, any two rings are separable without breaking ({\em
i.e.} any pair of rings is unlinked).

\begin{figure}[htp]
   \begin{center}
\begin{tabular}{ccccc}
      \includegraphics[width=2.5cm]{E.figsbw2/link01.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/link31.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/link41.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/2rings.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/borromean.pdf} \\
      (A) & (B) & (C) & (D) & (E) \\
\end{tabular}
   \end{center}
   \vspace{-0.7cm}
...
called a {\em knot diagram} (if only one component) or {\em link
...
\begin{figure}[htp]
   \begin{center}
\begin{tabular}{ccccc}
      \includegraphics[width=2.5cm]{E.figsbw2/link01d.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/link31d.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/link41d.pdf} &
      \includegraphics[width=2.5cm]{E.figsbw2/2ringsd.pdf}  &
      \includegraphics[width=2.5cm]{E.figsbw2/borromeand.pdf} \\
      (A) & (B) & (C) & (D) & (E) \\
\end{tabular}
   \end{center}
   \vspace{-0.7cm}
...
So, a link diagram can be seen as a 4-regular {\em plane graph} with
...
this figure is said to be the {\em overcrossing} ({\em
undercrossing}) line in the top case and the {\em undercrossing}
({\em overcrossing}) line in the bottom case.

\begin{figure}[htp]
   \begin{center}
   \includegraphics[width=10cm]{A.figs/linkdiagramelements.eps}
   \end{center}
   \vspace{-0.7cm}
...
\begin{figure}[htp]
   \begin{center}
   \includegraphics[width=10cm]{A.figs/exampleofambientisotopicknots.eps}
   \end{center}
   \vspace{-0.7cm}
...
\begin{figure}[htp]
   \begin{center}
   \includegraphics[width=15cm]{E.figsbw2/reidemeistermoves.pdf}
   \end{center}
   \vspace{-0.7cm}
...
Reidemeister \cite{reid1932} proved the following result
about link equivalence in the diagrammatic language:
\begin{center}\em two links are equivalent ({\em i.e.} ambient isotopic) if
...
\end{center}
We use the symbol $\equiv_A$ between two link diagrams or detached
...
$RE_2$ and $RE_3$ are said to be {\em regular isotopic}. The
notation $A \equiv_R B$, where $A$ and $B$ are link diagrams, is used
to say that $A$ and $B$ are regular isotopic. Note that regular
isotopic diagrams are always ambient isotopic,
$$ A \equiv_R B \implies A \equiv_A B,$$
...
relation on the set of link diagrams. This relation is called {\em
regular isotopy}.

Link diagrams interpreted as  {\em blackboard framed links}, as we
will see later, is a presentation for spaces ({\em i.e.} connected,
...
\subsection{Linking number, writhe and linking matrix}

A link is said {\em oriented} if all its components have an {\em
...
number +1 or -1 which is called the {\em sign of the (oriented)
...
\begin{figure}[htp]
   \begin{center}
   \includegraphics[width=9cm]{A.figs/orientedlinks.eps}
   \end{center}
   \vspace{-0.7cm}
...
(crossings of $\alpha$ with $\alpha$) on $D$ is said to be its {\em
...
its {\em linking number} and is denoted by $\ell k(\alpha,\beta)$.
...
$\alpha_n$. The {\em linking matrix} of $D$ is given by
...
\subsection{Framed links and blackboard framed links: encoding spaces}
\label{sec:bfl}
...
Consider the unknot on $\mathbb{S}^3$, {\em i.e.} a ring floating inside the
...
``nothing'', leading to the ``shape'' of $\mathbb{S}^3$ with a toroidal hole in it.
...
leads to a closed 3-manifold different from $\mathbb{S}^3$. In fact, this
idea generalizes.

Let's call a replacement like the one we mentioned above by {\em
surgery}. Think of a link on $\mathbb{S}^3$ and a thin tubular volume $T_i$
...
different closed 3-manifolds by different surgeries ({\em i.e.}
replacement of the thin tubular volumes $T_i$). In fact, as
Lickorish \cite{lickorish1962representation} and Wallace 
\cite{wallace1960modifications} proved
...
link on $\mathbb{S}^3$. So, by doing valid replacements of the thin tubular
volumes centered on the components of a link, one can obtain any
closed, connected, oriented 3-manifold. This result is very
important because it shows an intrinsic connection between links ({\em
i.e.} embeddings of circles into the 3-dimensional sphere $\mathbb{S}^1
\rightarrow \mathbb{S}^3$) with spaces ({\em i.e.} closed, connected,
oriented 3-manifold).
...
called {\em framing}. So, with a link and a framing for each of its
component, a space is defined. A framing as, is justified in Chapter
9 of \cite{rolfsen2003knots} may be just an integer number. This leads to
the definition we use of {\em framed link}: a link in $\mathbb{S}^3$ with an
...
\begin{center}
\em given any framed link, it is possible to draw a link
diagram\linebreak
 for it where the self-writhe of each component on the link diagram \linebreak
...
\begin{figure}[htp]
   \begin{center}
   \includegraphics[width=12cm]{A.figs/blackboardframedlink.eps}
   \end{center}
   \vspace{-0.7cm}
...
A {\em blackboard framed link} or {\em BFL} is a link diagram
...
diagram; (2) the framing of each component equals to the self-writhe of
...
hole in $\mathbb{S}^3$. But how is that done? Here is how.
A hole is a solid torus embedded in $\\mathbb{S}^3$.
...
\begin{figure}[htp]
   \begin{center}
   \includegraphics[width=7cm]{A.figs/unknotwithmeridian.eps}
   \end{center}
   \vspace{-0.7cm}
...
\subsection{A calculus on blackboard framed links}
...
\begin{figure}[htp]
   \begin{center}
      \leavevmode
      \includegraphics[width=15cm]{A.figs/kauffmancalculus.eps}
   \end{center}
   \vspace{-0.7cm}
...
$\equiv_S$ between two blackboard framed links denotes that both BFLs
induce the same space. When the symbol $\equiv_S$ is used between two
...
$\mathbb{S}^3$ defined by each of these BFL's, that the resulting space is
$\mathbb{S}^2 \times \mathbb{S}^1$ (See \cite{Rolfsen1976}).
\begin{figure}[htp]
   \begin{center}
      \leavevmode
      \includegraphics[width=13cm]{A.figs/examplebflcalculus.eps}
   \end{center}
   \vspace{-0.7cm}
   \caption{ Example of BFL's inducing the same
   space: $\mathbb{S}^2 \times mathbb{I}S^1$}
...
the {\em ribbon move}, and denoted by $K_1$, can replace the
infinite class of moves $K_5(n)$ on ${\cal K}^0$ leading to a
simpler and equivalent calculus ${\cal K}^1$.
\begin{figure}[htp]
   \begin{center}
      \leavevmode
      \includegraphics[width=8.5cm]{A.figs/ribbonmove.eps}
   \end{center}
   \vspace{-0.7cm}
   \caption{ The {\em ribbon move} or $K_1$}
...
\begin{lemma}[Whitney trick] Blackboard framed links that differ by
the pieces below are regular isotopic, so they induce the same
space.
\begin{center}
      \includegraphics[width=8cm]{A.figs/whitneytrickresult.eps}
\end{center}
...
transformation
\begin{center}
\includegraphics[width=12cm]{A.figs/whitneytrickproof.eps}.
\end{center}
Note that each passage is a regular isotopy move in $\{K_2, K_3\}$.
\end{proof}

\begin{proposition} The ribbon move follows from ${\cal K}^0$. More
...
\begin{center}
\includegraphics{A.figs/ribbonmoveproof.eps}
\end{center}
\vspace{-0.8cm}
\end{proof}

To show that $K_1$ actually can replace $K_5(n)$ it remains to prove
that with the remaining moves and $K_1$ ({\em i.e.} moves in$({\cal
...
\begin{figure}[htp]
   \begin{center}
      \leavevmode
      \includegraphics[width=13cm]{A.figs/somenotation.eps}
   \end{center}
   \vspace{-0.7cm}
...
\begin{lemma} Regular isotopy leads to
\begin{center}
\includegraphics{A.figs/ncurland2pibox.eps}
\end{center}
\end{lemma}
\begin{proof}
Generalization of the following case where $n = 3$.
\begin{center}
\includegraphics[width=15cm]{A.figs/ncurland2piboxproof2.eps}
\end{center}
\vspace{-0.8cm}
\end{proof}


\begin{lemma} Regular isotopy alone is capable of simplifying the left
configuration below with $2i^2$ crossings down to the right one with $2i$
crossings. \label{lem:istranistrands}
\begin{center}
\includegraphics[width=9cm]{A.figs/istrandistrands.eps}
\end{center}
\end{lemma}
\begin{proof}
This proof is taken from \cite{Kauffman1991}.
\begin{center}
\includegraphics[width=10cm]{A.figs/istrandistrandsproof.eps}
\end{center}

\vspace{-1cm}
\end{proof}

\begin{theorem} The ribbon move $K_1$ together with regular isotopy
...
\begin{center}
\includegraphics[width=9cm]{A.figs/ribbonmoveimpliesk5proof.eps}
\end{center}
...
\begin{figure}[htp]
   \begin{center}
      \leavevmode
      \includegraphics[width=12.5cm]{A.figs/bflcalculusk1.eps}
   \end{center}
   \vspace{-0.7cm}
...
\begin{lemma}[Passing Wall Lemma] These patterns are all regular isotopic

\begin{center}
\includegraphics{A.figs/passingwalllemma.eps}
\end{center}
\end{lemma}
\begin{proof}
This is just the idea: start passing
the horizontal curve under xor (i.e. exclusive or) over all
the crossings of the white ball using the moves $K_2$ and $K_3$.
\end{proof}

\begin{lemma}[Passing Cross Lemma] \label{lem:passingCrossLemma} The first
two and last two patterns are all regular isotopic
\begin{center}
\includegraphics{A.figs/passingcrosslemma.eps}
\end{center}
\end{lemma}
...
\begin{center}
\includegraphics{A.figs/passingcrosslemmaproof.eps}
\end{center}
\end{proof}

\begin{lemma}[Jumping Rope Lemma] \label{lem:jumpingRopeLemma}
The following patterns induces the same space.
\begin{center}
\includegraphics{A.figs/jumpingropelemma.eps}
\end{center}
\end{lemma}
\begin{proof} Follow the picture below. It proves that the first two
...
\begin{center}
\includegraphics{A.figs/jumpingropelemmaproof.eps}
\end{center}
\vspace{-0.8cm}
\end{proof}
